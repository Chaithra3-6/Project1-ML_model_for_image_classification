import streamlit as st
import numpy as np
import tensorflow as tf
from PIL import Image

# Custom Streamlit theme configuration
st.set_page_config(
    page_title="Image Classification App",
    page_icon="🖼️",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Inject custom CSS for gradient and button hover effect
st.markdown(
    """
    <style>
        body {
            background: linear-gradient(135deg, #F0FFFF, #89CFF0, #87CEEB, #F0FFFF);
            color: black;
        }
        .stButton button {
            background: linear-gradient(45deg, #87CEEB, #89CFF0 );
            color: white;
            border: none;
            border-radius: 10px;
            box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
            transition: transform 0.2s ease-in-out, box-shadow 0.2s;
        }
        .stButton button:hover {
            transform: scale(1.1);
            box-shadow: 0 8px 12px rgba(0, 0, 0, 0.2);
        }
        .stSidebar {
            background: linear-gradient(45deg, #F0FFFF, #89CFF0);
            color: black;
        }
        .stMetric label {
            color: black;
        }
        .stSidebar .stAlert {
            background-color: #E6E6FA;
            color: black;
            border-radius: 10px;
            padding: 10px;
            box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
        }
    </style>
    """,
    unsafe_allow_html=True
)

# Function for MobileNetV2 ImageNet model
def mobilenetv2_imagenet():
    st.title("🌐 ImageNet Classification with MobileNetV2")
    st.write("Upload an image to classify using the pre-trained MobileNetV2 model.")

    uploaded_file = st.file_uploader(
        "Choose an image...",
        type=["jpg", "png"],
        help="Upload a JPG or PNG image for classification"
    )

    if uploaded_file is not None:
        # Display the uploaded image
        col1, col2 = st.columns(2)

        with col1:
            image = Image.open(uploaded_file)
            # Convert image to RGB if it has an alpha channel
            if image.mode != 'RGB':
                image = image.convert('RGB')
            st.image(image, caption='Uploaded Image', use_container_width=True)

        with col2:
            # Placeholder for status update
            status_placeholder = st.empty()

            # Set initial status
            status_placeholder.write("Classifying...")

            # Load MobileNetV2 model
            model = tf.keras.applications.MobileNetV2(weights='imagenet')

            # Preprocess the image
            img = image.resize((224, 224))
            img_array = np.array(img)
            img_array = np.expand_dims(img_array, axis=0)
            img_array = tf.keras.applications.mobilenet_v2.preprocess_input(img_array)

            # Make predictions
            predictions = model.predict(img_array)
            decoded_predictions = tf.keras.applications.mobilenet_v2.decode_predictions(predictions, top=3)[0]

            # Update the status to "Classification complete!"
            status_placeholder.write("Classification complete!")

            # Display top 3 predictions
            st.subheader("Top 3 Predictions:")
            for i, (imagenet_id, label, score) in enumerate(decoded_predictions, 1):
                st.metric(
                    label=f"{i}. {label.capitalize()}",
                    value=f"{score * 100:.2f}%"
                )

# Function for CIFAR-10 model
def cifar10_classification():
    st.title("🖼️ CIFAR-10 Image Classification")
    st.write("Upload an image to classify using custom CIFAR-10 trained model.")

    uploaded_file = st.file_uploader(
        "Choose an image...",
        type=["jpg", "png"],
        help="Upload a JPG or PNG image for classification"
    )

    if uploaded_file is not None:
        # Display the uploaded image
        col1, col2 = st.columns(2)

        with col1:
            image = Image.open(uploaded_file)
            # Convert image to RGB if it has an alpha channel
            if image.mode != 'RGB':
                image = image.convert('RGB')
            st.image(image, caption='Uploaded Image', use_container_width=True)

        with col2:
            # Placeholder for status update
            status_placeholder = st.empty()

            # Set initial status
            status_placeholder.write("Classifying...")

            # CIFAR-10 class names
            class_names = [
                'airplane', 'automobile', 'bird', 'cat', 'deer',
                'dog', 'frog', 'horse', 'ship', 'truck'
            ]

            # Load CIFAR-10 model
            model = tf.keras.models.load_model('model111.h5')

            # Preprocess the image
            img = image.resize((32, 32))
            img_array = np.array(img)
            img_array = img_array.astype('float32') / 255.0
            img_array = np.expand_dims(img_array, axis=0)

            # Make predictions
            predictions = model.predict(img_array)
            predicted_class = np.argmax(predictions, axis=1)[0]
            confidence = np.max(predictions)

            # Update the status to "Classification complete!"
            status_placeholder.write("Classification complete!")

            # Display prediction
            st.subheader("Prediction Results:")
            st.metric(
                label="Predicted Class",
                value=class_names[predicted_class]
            )
            st.metric(
                label="Confidence",
                value=f"{confidence * 100:.2f}%"
            )

# Main function to control the navigation
def main():
    # Custom sidebar styling
    st.sidebar.title("🔍 Model Selection")
    st.sidebar.write("Choose an image classification model:")

    choice = st.sidebar.radio(
        "Select Model",
        ("CIFAR-10", "MobileNetV2 (ImageNet)"),
        key="model_select"
    )

    # Add a separator
    st.sidebar.markdown("---")

    # Additional information in sidebar
    st.sidebar.info(
        "💡 Upload an image and let my AI classify it!\n\n"
        "- CIFAR-10: Classifies 10 specific object categories\n"
        "- MobileNetV2: Recognizes 1000 ImageNet categories"
    )

    # Run selected model
    if choice == "MobileNetV2 (ImageNet)":
        mobilenetv2_imagenet()
    elif choice == "CIFAR-10":
        cifar10_classification()

# Run the app
if __name__ == "__main__":
    main()
